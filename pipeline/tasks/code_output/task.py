...
class CodeOutputTask(BaseTask):
    ...
    def compute_metrics(self, results: list[dict]) -> dict:
        ...
        dist_by_variant = defaultdict(
            lambda: {"count": 0, "correct": 0, "incomplete": 0, "wrong": 0}
        )
    ...
    def format_metrics(self, metrics: dict, model_name: str | None = None) -> str:
        ...
        lines.append(f"{'Variant':<16} {'Count':>7} {'Correct':>10} {'Incomplete':>12} {'Wrong':>8}")
        lines.append("-" * 55)
        ...
        for variant in [v for v in variant_order if v in dist_by_variant] + sorted(set(dist_by_variant) - set(variant_order)):
            ...
            lines.append(
                f"{variant:<16} {d['count']:>7} {d['correct']:>10} {d['incomplete']:>12} {d['wrong']:>8}"
            )

        lines.append("-" * 55)
        d = metrics.get("distribution", {})
        lines.append(
            f"{'Total':<16} {d.get('count', 0):>7} {d.get('correct', 0):>10} {d.get('incomplete', 0):>12} {d.get('wrong', 0):>8}"
        )
