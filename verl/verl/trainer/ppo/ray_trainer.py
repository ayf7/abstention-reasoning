...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch.non_tensor_batch["epsilon"] = np.array([0 for _ in range(len(test_gen_batch.batch))])
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                def exponential_decay(t, A=0.9, k=12.43):
                    return A * np.exp(-k * t)
                
                def bell_decay(t):
                    return 2 * t * np.exp(-4 * t) + 0.2 
                step_fraction = float(self.global_steps)/float(self.total_training_steps)

                if self.config.trainer.use_epsilon:
                    epsilon = exponential_decay(step_fraction, self.config.trainer.epsilon_A, self.config.trainer.epsilon_k)
                    #epsilon = bell_decay(step_fraction)
                else:
                    epsilon = 0
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        gen_batch.non_tensor_batch["epsilon"] = np.array([epsilon for _ in range(len(batch.batch))]) #np.array([epsilon for _ in range(len(batch.batch))])
                        if not self.async_rollout_mode:
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            #num_hints = self.tokenizer.batch_decode(batch.batch["responses"], skip_special_tokens=True)
