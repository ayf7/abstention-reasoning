    ...
    async def generate_response(
        ...
        **kwargs,
        ...
        if hint_idx < len(hints):
            ...
            response = f"<response>{hint}</response>"
            logger.debug(f"Providing hint {hint_idx + 1}/{len(hints)}: {hint}")

            # Continue the interaction (model should keep reasoning)
            return False, response, 0.0, {"num_hints": inst["num_hints_given"], "hint_provided": hint}
        else:
            # No more hints available
            response = "<warning>No more hints available. Please provide your final answer.</warning>"
            logger.debug(f"No more hints available (requested {hint_idx + 1}, have {len(hints)})")
